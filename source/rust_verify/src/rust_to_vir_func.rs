...
fn compare_external_ty<'tcx>(
    ...
    {
        compare_external_ty(tcx, verus_items, &ty1, &ty2, external_trait_from_to)
    } else if let (rustc_middle::ty::TyKind::RawPtr(ty1, _), rustc_middle::ty::TyKind::RawPtr(ty2, _)) =
        (ty1.kind(), ty2.kind())
    {
        compare_external_ty(tcx, verus_items, &ty1, &ty2, external_trait_from_to)
    } else if let (rustc_middle::ty::TyKind::Ref(_, ty1, _), rustc_middle::ty::TyKind::Ref(_, ty2, _)) =
        (ty1.kind(), ty2.kind())
    {
        compare_external_ty(tcx, verus_items, &ty1, &ty2, external_trait_from_to)
    }  else {
