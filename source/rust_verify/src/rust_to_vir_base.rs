    ...
    for d in def_path.data.iter() {
        ...
        match &d.data {
            ...
            DefPathData::OpaqueTy => {
                segments.push(Arc::new(vir::def::RUST_OPAQUE_TYPE.to_string() + &d.disambiguator.to_string()));
            }
            _ => return None,
        }
    }
...
pub(crate) fn mid_ty_to_vir_ghost<'tcx>(
    ...
    let t = match ty.kind() {
        ...
        TyKind::Alias(rustc_middle::ty::AliasTyKind::Opaque, al_ty) => {
            let id: String = format!("opaty_{}_{}_{:?}",al_ty.def_id.krate.as_u32(), al_ty.def_id.index.as_u32(), al_ty.args).chars()
                .filter(|c| !matches!(c, '[' | ']' | '"' | '\'' | ':' | '/' | '#' | ' ' | ','))
                .collect::<String>().to_uppercase();
            let instantiated_bounds = tcx.item_bounds(al_ty.def_id).instantiate(tcx, al_ty.args);
            ...
            for bound in instantiated_bounds{
                 match bound.kind().skip_binder() {
                    ClauseKind::Trait(TraitPredicate {
                        ...
                        let trait_def_id = trait_ref.def_id;
                        let generic_bound = check_opaque_ty_generic_bound(
                            tcx, 
                            verus_items, 
                            param_env_src, 
                            span, 
                            trait_def_id, 
                            substs, 
                            &Arc::new(TypX::TypParam(Arc::new(id.clone())))
                        )?.unwrap();
          ...
          (Arc::new(TypX::Opaque{id: Arc::new(id.clone()), trait_bounds: Arc::new(trait_bounds)}), false)
        }
...
pub(crate) fn check_opaque_ty_generic_bound<'tcx>(
    tcx: TyCtxt<'tcx>,
    verus_items: &crate::verus_items::VerusItems,
    param_env_src: DefId,
    span: Span,
        ...
        || Some(trait_def_id) == tcx.get_diagnostic_item(rustc_span::sym::Send)
    {
        ...
        let mut vir_args = vec![self_ty.clone()];
