            ...
            let mut stmts = if skip {
                ...
                let mut stmts = if let Some(dest_id) = state.post_condition_info.dest.clone() {
                    ...
                    let mut stmts = if is_async_fn == false {
                        ...
                        async_fn_return_to_stmts(ctx, state,expr_ctxt, &mut ret_typ_op, &async_body_return_type, ret_exp, &stm, dest_id)?
                    };
                    

                    // if return value exists, check if we need to emit additional assumes for nested opaque types
                    if ret_typ_op.is_some() {
...
fn try_reveal_opaque_ty_ctor(
    exp: &Exp,
) -> Typ
{
    match &exp.x{
        ExpX::Ctor(Dt::Tuple(len), _, items) => {
            Arc::new(TypX::Datatype(Dt::Tuple(*len),Arc::new(items.iter().map(|x| 
                try_reveal_opaque_ty_ctor(&x.a)
            ).collect()) , Arc::new(vec![])))
        },
        ExpX::UnaryOpr(_, exp) => {
            try_reveal_opaque_ty_ctor(exp)
        }
        ExpX::If(_, exp, _) => {
            try_reveal_opaque_ty_ctor(exp)
        }
...
fn async_fn_return_to_stmts(
    ...
    async_body_return_type: &Option<Arc<TypX>>,
...
) -> Result<Vec<Stmt>, VirErr>
{
    ...
    // let proj_typ_ids = typ_to_ids(ctx, &proj_typ);
    // let ret_exp_ids = typ_to_ids(ctx,&ret_exp.typ);
    ...
    *ret_op = Some(async_body_return_type.clone());
    ...
    // Arc::new(StmtX::Assume(assume_dcr.into())),
    //  Arc::new(StmtX::Assume(assume_typ.into()))
     ])
}
